...
var tooltip = body.append('div').attr('class', 'tooltip').attr('id', 'tooltip').style('opacity', 0);

var svg = d3.select('#tree-map'), width = +svg.attr('width'), height = +svg.attr('height');
...
d3.json(DATASET.FILE_PATH)
    .then((data) => {
        ...
        var cell = svg.selectAll('g').data(root.leaves()).enter().append('g').attr('class', 'group').attr('transform', function (d) {
        ...
        cell.append('rect').attr('id', function (d) {
            ...
            tooltip.html(
                ...
                '<br>Value: ' + 
                d.data.value
            ).attr('data-value', d.data.value).style('left', event.pageX - 10 + 'px').style('top', event.pageY + 28 + 'px');
        ...
        cell
        .append('text').attr('class', 'tile-text').selectAll('tspan').data(function (d) {
